...
// Control Limits
const float maxSpeed = 10; // maximum velocity in m/s
...
int escPwmFromMetersPerSecond(float velocity) {
  ...
  else if(velocity < SpeedLUT[SpeedLUTMaxIndex][1])
  {
...
int escPwmPID(float velocity) {
  if (velocity <= 0) {
    ...
    if (kI * integral < maxIntegralPwm || trapezoidIntegral < 0) {
